...
from src.recording.service import RecordingService
...
async def start_diagnose(msg: Message, state: FSMContext):
    ...
    await state.set_state(Diagnose.waiting_voice)


@router.message(Diagnose.waiting_voice, F.text == "Меню")
async def return_to_menu(msg: Message, state: FSMContext):
    await state.clear()
    await msg.answer("🔙 Возвращаемся в главное меню! Запись прервана.", reply_markup=kbs_menu)


@router.message(Diagnose.waiting_voice, F.voice)
...
@router.message(Diagnose.waiting_voice)
async def handle_not_voice(msg: Message):
    await msg.answer("Пожалуйста, пришли именно *голосовое сообщение*.", reply_markup=kb_menu)
