...
from src.keyboards.sessions import get_session_kbs
from src.diagnostic.utils import get_user_id_by_tg_id
from src.recording.repositories import RecordingRepository
...
async def send_for_diag(paths: list[str]):
    # Transform relative paths to absolute paths for the AI service
    # The AI service expects paths relative to /app/records
    transformed_paths = []
    for path in paths:
    ...
    async with httpx.AsyncClient(timeout=60) as c:
        resp = await c.post("http://ai_predict_service:8000/diagnose", json=transformed_paths)
...
async def handle_session_result(msg: Message):
    ...
    if not agreed:
        await show_terms_agreement(
            msg, msg.bot.fsm.get_context(bot=msg.bot, user_id=msg.from_user.id, chat_id=msg.chat.id)
        )
        return
